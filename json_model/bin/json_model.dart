...
bool generateModelClass(
  ...
  fileList.forEach((f) {
    if (FileSystemEntity.isFileSync(f.path)) {
      ...
      final String constructorFields = fields.toString();

      List<String> constructorField = constructorFields.split(';');
      String finalConstructor = constructorField
          .toString()
          .replaceAll('[', '')
          .replaceAll(']', '')
          .replaceAll(RegExp(r'(\/)(.*?)+'), '')
          .replaceAll('\n', '')
          .trimRight();
      final List<String> createParameters = finalConstructor.split(",");
      String finalized = '{';
      createParameters.forEach((element) {
        if (element.isNotEmpty) {
          finalized += 'this.' + element.split("?")[1].trim() + ', ';
        }
      });
      finalized = finalized.substring(0, finalized.length - 2) + '}';
